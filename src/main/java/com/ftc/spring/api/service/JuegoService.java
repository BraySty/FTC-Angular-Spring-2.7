...
import com.ftc.spring.api.repository.JuegoRepository;
...
public class JuegoService {
    ...
    public ResponseEntity<?> readByTitleOrDeveloper(String title, String developer) {
        if (developer == null && title == null) {											
			...
			if (posibleJuego.isPresent()) {
				...
				return new ResponseEntity<>(new Mensaje("El juego con titulo " + title + noExist), HttpStatus.NOT_FOUND);
			}
		} else {
			...
			if (posibleJuego.isPresent()) {
				...
				return new ResponseEntity<>(new Mensaje("El desarrollador con nombre " + developer + noExist), HttpStatus.NOT_FOUND);
			}
		}
    }
    ...
    public ResponseEntity<?> readByID(Integer idJuego) {
		...
		if (posibleProducto.isPresent()) {
			...
			return new ResponseEntity<>(new Mensaje("El juego con id: " + idJuego + noExist), HttpStatus.NOT_FOUND);
		}
    }
